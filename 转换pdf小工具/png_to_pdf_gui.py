...
class PNGtoPDFConverter:
    ...
    def convert_to_pdf(self):
        ...
        if not self.png_files:
            messagebox.showerror(title="错误", message="请选择PNG文件", parent=self.master)
            return
        ...
        try:
            # 打开所有PNG文件
            for file in self.png_files:
                img = Image.open(file).convert('RGB')  # 确保图像为RGB模式
                images.append(img)
            ...
            if images:
                ...
                messagebox.showerror(title="错误", message="没有可转换的图像", parent=self.master)
